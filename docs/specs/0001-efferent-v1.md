...
3. ~~自动化/定时任务管理~~ **（2026-08-16 探察门结论：移出 v1）**——引擎 schedule 仅暴露为模型工具（无 HTTP/命令/暂停，见 docs/notes/engine-api.md），外部面板不可行；正解是随应用分发宿主端插件注册 HTTP 路由，与插件分发机制、语音输入一起归入 SPEC-0002
4. 语音输入（🔊/🎤 已验证的浏览器端插件）：第二梯队（归 SPEC-0002）
